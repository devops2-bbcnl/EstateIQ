...
export default function NewsletterForm({ variant = 'default', source: sourceProp }: NewsletterFormProps) {
  ...
  if (done) {
    return (
      <p className={`text-sm ${isFooter ? 'text-muted-foreground' : 'text-emerald-200'}`}>
  ...
  return (
    <form onSubmit={handleSubmit} className={isFooter ? 'space-y-3' : 'space-y-4'}>
      <div>
        ...
        <p
          className={
            isFooter ? 'text-xs text-muted-foreground mb-3' : 'text-sm text-white/90 mb-3'
          }
        >
          ...
          <input
            ...
            className={
              isFooter
                ...
                : 'flex-1 rounded-lg border border-white/30 bg-white/95 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/40'
            }
          />
      ...
      {error ? (
        <p className={`text-xs ${isFooter ? 'text-destructive' : 'text-red-200'}`}>{error}</p>
      ) : null}
    </form>
  )
}
